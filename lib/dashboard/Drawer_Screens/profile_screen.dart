...
// e.g. "MONT JUNIOR" → "Mont Junior"  |  "royal fan" → "Royal Fan"
// Only formats at the display level; the original API value is never mutated.
...
class ProfileScreen extends StatelessWidget {
  ...
  Future<Map<String, String>> _loadProfileData() async {
    ...
    return {
      ...
      'id': id,
      'class':
          studentData?.className ?? userData?['classDesc']?.toString() ?? '',
      'section':
          studentData?.section ?? userData?['section']?.toString() ?? '',
      'fatherName':
          studentData?.fatherName ?? userData?['father_Name']?.toString() ?? '',
      'rollNo': studentData?.rollNo != null
          ? studentData!.rollNo.toString()
          : (userData?['rollNo']?.toString() ?? id),
    };
                                ...
                                child: Column(
                                  children: [
                                    Text(
                                      ...
                                      style: TextStyle(
                                        fontSize:
                                            (screenWidth * 0.07).clamp(22.0, 30.0),
                                        fontWeight: FontWeight.bold,
                                        color: Theme.of(context)
                                            .colorScheme
                                            .onSurface,
